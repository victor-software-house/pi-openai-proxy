...
1. Resolve the active feature from the native `/spec` runtime and collect the prepared absolute paths for:
   - `FEATURE_DIR`
   - `FEATURE_SPEC`
   - (Optionally capture `IMPL_PLAN`, `TASKS` for future chained flows.)
   - If the active feature cannot be resolved, abort and instruct the user to re-run `/spec specify` or verify feature branch state.
